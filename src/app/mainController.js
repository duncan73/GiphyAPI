...
export default function(giphyService) {
  ...
  function mapResults(results) {
    results.then(
        function(results) {
          main.results = _.map(results.data.data, item => {
            return {
              id: item.id,
  ...
  main.getKittens = function(offset) {
    ...
    if (lastCallToApi !== main.getKittens) {
      main.offset = 0;
    }
  ...
  main.getPuppies = function(offset) {
    ...
    if (lastCallToApi !== main.getPuppies) {
      main.offset = 0;
    }
  ...
  main.getFavourites = function() {
    giphyService.getFavourites().then(
        function(results) {
